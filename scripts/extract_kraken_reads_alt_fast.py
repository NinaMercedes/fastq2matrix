...
def main():
    ...
    parser.add_argument('-e', "--extra-taxid",dest='extra_taxid',
        nargs='+',
        help='Taxonomy ID[s] of reads to extract (space-delimited)')
    ...
    if args.parents or args.children:
        ...
        if args.extra_taxid:
            for tid in args.extra_taxid:
                sys.stdout.write("Adding extra taxid %s\n" % tid)
                save_taxids[int(tid)] = 0
    ##############################################################################
    sys.stdout.write("SAVING READS FOR TAXIDS:")
    for tid in save_taxids:
        sys.stdout.write(" %i" % tid)
    ...
    with open(tmpfile,"w") as O:
        for x in save_readids:
            O.write("%s\n" % x)
    #Sequence files
    seq_file1 = args.seq_file1
    seq_file2 = args.seq_file2
    ####TEST IF INPUT IS FASTA OR FASTQ
    sys.stdout.write(f"Writing {args.output_file}\n")
    sys.stdout.write(f"Writing {args.output_file2}\n")
    sp.call(f"seqtk subseq {seq_file1} {tmpfile} > {args.output_file}",shell=True)
    sp.call(f"seqtk subseq {seq_file2} {tmpfile} > {args.output_file2}",shell=True)
    sp.call(f"rm {tmpfile}",shell=True)
    # if(seq_file1[-3:] == '.gz'):
